...
def apply_primitive(procedure, args, env):
    ...
    while i < args.__len__():
        argList.append(args.__getitem__(i))
        i += 1
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        name = target[0]
        if target.__len__() >= 3:
            targetEdited = Pair(vals.first.second.first, vals.first.second.second)       
            valsEdited = Pair(targetEdited, vals.second)
            lambdaEq = do_lambda_form(valsEdited, env)
            env.bindings[name] = lambdaEq
        elif target.__len__() == 1:
            valsEdited = Pair(nil, vals.second)
            lambdaEq = do_lambda_form(valsEdited, env)
            env.bindings[name] = lambdaEq
        else:
            valsEdited = Pair(target.second, vals.second)
            lambdaEq = do_lambda_form(valsEdited, env)
            env.bindings[name] = lambdaEq
        return name
    else:
...
def do_begin_form(vals, env):
    ...
    while i < vals.__len__():
        if i == finalIndex:
            return vals[i]
        toReturn = scheme_eval(vals[i], env)
        i += 1
    ...
    for element in formals:
        if (scheme_symbolp(element)):
            if (element not in checked):
                ...
                raise SchemeError("Found repeated symbol in formals")
        else:
            raise SchemeError("Found element which is not a symbol in formals")
